...
- [ ] 0.1 Confirm you're in `~/dev/working-trees/tuscany-trip-collioure-fox` on `feature/collioure-trip`
      (the planner cut it off `origin/main`; the research + plan docs are already here on the branch).
- [ ] 0.2 Push an **empty starter commit** first for board liveness (RECIPES §Worktree-per-pup):
...
- [ ] 1.1 Copy `france.html` → `collioure.html` as the starting point.
- [ ] 1.2 **Head:** keep the Clarity block verbatim; set `<title>` = **"Collioure & the Côte Vermeille · Helen & Alex"**;
      update any `og`/meta/description to mirror siblings (Collioure subject). Keep all font + CSS links.
- [ ] 1.3 **Palette:** re-tune the inline `:root` custom props toward terracotta-and-sea (keep
...
- [ ] 2.1 **Hero:** Wikimedia Collioure image with `onerror`; eyebrow "Option eleven · Collioure & the
...
- [ ] 2.2 **Intro:** `.lead` (with an `<em>` highlight) + `.body` paragraph framing Collioure around
      **Fauvism/art, swimming, markets, coastal walks, the bell-tower & fort** — never drink.
      Keep the `.play-journey-btn`.
- [ ] 2.3 **Facts:** 4-up `.fact` strip (e.g. "1 base — Collioure, on the line" / "Fauvism — where
      Matisse & Derain invented it" / "rail · bus · boat — 100% car-free" / "high-20s — sea-breeze coast").
- [ ] 2.4 **Timeline (`~12–13 days`, single base):** author `.t-item`s. Day 1 = down to the south
...
- [ ] 2.5 **"Where we'd stay" base block:** the `.base` → `.base-grid` (`.base-img` + `.base-txt`)
...
- [ ] 2.6 **"Big hitters" cards grid:** one `.grid` of `.card` day-trip anchors (fold france's two
...
- [ ] 2.8 **Footer:** mirror france's `<footer>` (a "Tempted?" big line + a paragraph + the
...
- [ ] 3.1 Author the `stops[]` array at the bottom (research §4 has coords). **Collioure = the single
      `{ base:true, coords:[42.5263,3.0830], … }`.**
- [ ] 3.2 **Edinburgh + Paris = `{ kind:"travel", … }`** (straight approach legs).
- [ ] 3.3 Any **rest/beach-at-home day** at Collioure coords = `{ kind:"travel", coords:[42.5263,3.0830], … }`
      (anchor at home, no spurious arc — exactly as france.html does for its Day-5 rest day).
- [ ] 3.4 Every **day-trip stop** (Banyuls, Céret, Perpignan, Port-Vendres, Cadaqués, Argelès, …) has
      **NO `kind`** → it auto-spokes. Keep `eyebrow`/`title`/`desc`/`chips` populated like france.
- [ ] 3.5 Sanity-check: exactly one `base:true`; approach legs flagged travel; no day-trip wrongly
      flagged travel.

### Phase 4 — `index.html` wiring
- [ ] 4.1 Add gradient rule in the `.opt-ph` block (after the `.opt--cap` line ~106):
...
- [ ] 4.2 Add accent rule in the `--ac` block (after the `.opt--cap` line ~229):
      `.opt--collioure{--ac:224,138,58}` (tune to match chosen gradient).
- [ ] 4.3 Clone the **Option 10 (cap)** card `<a>` block → make it
...
- [ ] 4.4 Bump the summary copy lower in `index.html`: **"Ten plans, all ready…" → "Eleven plans…"**
      (both the `<h2>` and the surrounding sentence). Check for any other "ten"/"10 options" count.

### Phase 5 — verify
- [ ] 5.1 **Playwright load check** (use the browser tool / a headless script): open `collioure.html`
...
- [ ] 5.2 **Grep gates** (must return NOTHING) on `collioure.html`:
      ```bash
      ...
      describe any leg.)
- [ ] 5.3 **`<img>` audit:** every `<img>` in `collioure.html` has an `onerror`
      (`grep -c "<img" collioure.html` == `grep -c "onerror" collioure.html` for the img lines; cards
      use the two-arg form).
- [ ] 5.4 **Size sanity:** `collioure.html` lands in the sibling band (~29–36 KB).
- [ ] 5.5 Update **this plan's Deviation log** with anything that diverged, then commit research + plan
